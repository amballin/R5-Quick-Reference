...
The first Camera Lab header row shows the active **Canon EDSDK/Simulated camera** backend, **Use Simulator/Use Camera**, **Enable camera changes/Return to read-only** in EDSDK mode, and **Stop Camera Lab**. The second row shows the change boundary and **Profile Pack:** followed by the live manifest name. A dedicated third row right-aligns `Camera Lab Major.Minor.Incremental · Main/Prototype` with the buttons above; expand it to see the full branch and diagnostic source hash. The redundant yellow checkout badge is not shown. The silver camera logo remains in its own adjacent column. Confirm the pack, version context, and change-state badges before physical-camera work.
...
Profile Editor can also start or reuse Camera Lab through **Open in Camera Lab**. It passes the current saved Subject/Profile Card name and the exact active resolved pack context. Camera Lab validates the pack, loads its current catalog, and preselects the profile; it does not connect, scan, compare, or write automatically. A running Lab is reused only when its pack ID matches. The two apps remain independent, so each Stop button closes only its own server and Camera Lab alone owns the camera session.

For an external pack, Camera Lab supports Steps 5A–5B connection, scan, comparison, C1–C3 labels, equipment context, setup routes, guarded simulator application, and the separately confirmed **Enable camera changes** path with reversible physical qualification. It displays the live manifest name and keeps machine-local journals, qualification, evidence, confirmations, and checklist state under the pack ID. Step 5C adds deliberate Profile Editor promotion of exact completed physical-camera evidence into pack-owned verification status; Camera Lab itself still cannot write pack source. Use Profile Editor to launch the external-pack Lab. The direct diagnostic equivalent is:

```bash
./80\ Build/scripts/start-camera-lab.sh --profile-pack "/absolute/path/to/private-profile-pack"
```
...
After the first comparison, review **Equipment context** before following the settings list:

- With a physical camera, Camera Lab selects the recognized camera-reported lens automatically. With the simulator or no recognized attached-lens match, it starts from the card's Primary lens.
- Use **Lens for this comparison** to plan with another lens/accessory combination authored on that card. If it differs from a physically attached lens, Camera Lab labels it as a planning override; attach that lens and rescan before applying the profile.
- When the selected lens has a Mode 1 / 2 / 3 switch, Camera Lab offers only the modes that lens supports. **Profile default** follows the merged card target—for example, Mode 3 for Birds in Flight. Choose another supported mode only as a deliberate field override.
- A lens such as the RF 24–240mm that has IS On/Off but no mode selector shows automatic stabilization behavior and removes the inapplicable Mode 1 / 2 / 3 action. A non-IS lens retains the camera-body stabilization route.
- Read **What this equipment or setting combination changes** and the corresponding notes on affected findings. Camera Lab marks inactive or equipment-overridden settings as no-action rows and retains explicit warnings or changed routes for restricted, replaced, coordinated, available, and automatic behavior.

The application flow is intentionally plain:

1. **Check that the camera is ready.** Confirm identity, power, equipment, competing applications, and recovery backup. Lens starts with the attached camera-reported lens, or the card's Primary lens when no attached lens is available; its menu contains only authored choices for that card. Flash offers **None** (the default), **Flash Attached**, or **Trigger**. Cards offers **CFexpress & SD**, **CFexpress**, or **SD**. If Camera Setup Essentials is already set, confirm that prompt to clear only matching Set & Forget targets that Camera Lab cannot verify directly; readable differences and profile-specific targets remain in the review. Changing a readiness choice while the review is still planned refreshes the review automatically. Once Apply starts, those controls show the recorded plan and remain locked; stop the attempt and create a new review to change them.
...
After **Scan & compare** completes in simulated mode, choose **Apply this profile to camera**. Complete the same readiness and review flow, including the controlled lens, flash, and card choices. Optionally confirm that Camera Setup Essentials is already set; Camera Lab uses that as manual evidence only for matching Set & Forget targets without contradictory readback. The page clearly identifies simulator mode and cannot change the physical EOS R5.
...
The Phase 1 read-only comparison, lens-aware equipment context, structured feature-interaction evaluation, manual checklist, guided C1–C3 maintenance and recalled-state validation, simulator rehearsal, and evidence-gated physical camera-change workflow are implemented. The ordinary Lab provides **Enable camera changes**, **Apply this profile to camera**, a plain readiness and review flow, one visible action at a time, an explicit completion receipt, and **Return to read-only**. Physical session 4 qualified Picture Style Neutral and Standard for the tested EOS R5 body/firmware/EDSDK context and restored Neutral. Physical session 5 on September 4, 2026 repeated the reversible Picture Style Standard-to-Neutral write, target readback, restoration to Standard, and restoration readback on firmware 2.2.1 with EDSDK 13.20.20.0 in both embedded and external-pack contexts; the external journal recorded the active pack identity, the camera was subsequently returned manually to Neutral, and no application or pack source changed. Every other exact value remains unavailable for automatic change until it passes the safety test. Physical session 3 manually registered and camera-body verified C1 Wildlife, C2 Birds in Flight, and C3 Landscape, with `C123_CFG.CSD` saved as the recovery checkpoint. Automated C1–C3 registration and camera-settings-data transfer remain unavailable; EOS R5 backups remain card-based.
