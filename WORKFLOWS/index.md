...
Open **R5 Camera Lab.app** from the machine-local `Applications` folder to connect a physical EOS R5 through read-only Canon EDSDK, or use **Open in Camera Lab** from a saved Subject/Profile Card in Profile Editor to start or reuse it with that profile preselected. Use [USB Camera Configuration](usb-camera-configuration.html) for the complete workflow. Choose **Enable camera changes**, then **Apply this profile to camera**. Camera Lab removes already-correct rows from the work queue, handles simulator-safe changes automatically after confirmation, groups manual work by camera route, and uses one rescan per group. The stationary **Do this now** card shows every exact target without requiring the complete-list view. Rapid setup receives each group result immediately and may reuse an identical manual setting and target across cards only within the same unchanged connected-camera session. Camera Lab reports either **Profile applied successfully** or **Profile not fully applied**. Choose **Return to read-only** when finished. Profile Editor and Camera Lab stop independently.
