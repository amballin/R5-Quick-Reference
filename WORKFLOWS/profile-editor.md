...
**Setup & Sharing** remains available with either source mode. With an external pack active, **New Profile Pack** explains why creation is paused and provides **Switch to embedded sources**. With embedded sources active, enter a friendly name and choose **Choose with Finder…**. The native macOS window provides normal folder navigation and New Folder support and returns the exact destination; the path field is not typed manually. Resolve all browser drafts, review the generated manifest, UUID, destination, source inventory, and Git boundary, then confirm separately. The editor stages and validates the complete pack, initializes local Git without a commit or remote, atomically installs it, registers it, selects it, and reloads. It never overwrites an existing destination, and embedded sources remain unchanged.

With an external pack active, **Private Pack Git & Handoff** keeps its repository operations separate from application Finish Day. Status names the application and pack independently. A new pack's source-bound one-use commit review must include its repository-owned `AGENTS.md`; the later commit confirmation changes only the pack and does not push. Exact credential-free HTTPS or SSH `origin` configuration uses another review and confirmation. Push uses a third confirmation, targets only the current pack branch's same-named `origin` branch, runs without interactive credential prompts, and stops on remote-only or divergent history. It never creates, switches, or force-pushes a branch.

Combined handoff is read-only and becomes ready only when both repositories are clean and each local head equals the live head of its exact matching origin branch. Checking the application remote does not fetch or mutate application Git. Application changes continue through normal Finish Day; the external-pack workflow cannot commit or push them.
