...
**Profile Packs** remains available with either source mode and opens **Profile Packs & Sharing**. With an external pack active, **New Profile Pack** explains why creation is paused and provides **Switch to embedded sources**. With embedded sources active, the editor shows the required baseline-backed C1–C3 and permanent-reference seed and offers every remaining application-catalog subject card as an unchecked optional choice. Enter a friendly name and choose **Choose with Finder…**. The native macOS window provides normal folder navigation and New Folder support and returns the exact destination; the path field is not typed manually. Choose a separate sibling folder rather than putting one profile pack inside another. Resolve all browser drafts, review the generated manifest, UUID, destination, required and selected cards, exact source inventory, and Git boundary, then confirm separately. The editor copies only the selected profile YAML, intersects lens guidance with active subject cards, clears camera verification/session history, and marks starter controls and registrations pending camera verification. It stages and validates the complete pack, adds a root `.gitignore` excluding `.DS_Store`, initializes local Git without a commit or remote, atomically installs it, registers it, selects it, and reloads. Pack resolution rejects a root containing another `profile-pack.yaml`, while creation rejects any destination below an existing pack manifest. Pack fingerprinting, review, and staging also omit Finder metadata for older packs. It never overwrites an existing destination, and embedded sources remain unchanged.

With an external pack active, **Add Profiles from Catalog** lists application-catalog subject profiles not already installed. Select one or more, resolve all browser drafts, and review the exact new profile YAML plus matching lens-guidance additions. A separate compact confirmation applies an add-only transaction: the editor rechecks application and pack fingerprints, rejects an existing filename or card identity, creates a pack-namespaced recovery backup, validates the combined candidate source, writes atomically, and rolls back on failure. It never replaces or removes an existing profile. The result is a local private-pack source change and is not committed or pushed automatically.

With an external pack active, Profile Packs & Sharing is a four-step state-driven flow: create or select, save locally, connect GitHub, and push and verify. Only the current required action is expanded. A new pack's source-bound one-use commit review must include its repository-owned `AGENTS.md`; the later compact confirmation changes only the pack and does not push. The connection step instructs the owner to create an empty private GitHub repository without README, `.gitignore`, or license, recommends HTTPS on macOS, explains the SSH-key prerequisite, and prohibits credentials in the URL. Reviewing a new exact URL can replace an unreachable old origin without contacting it first.
...
After the tracked ownership contract reaches `main`, complete the one-time [Application Owner Protection](application-owner-protection.html) ruleset setup. CODEOWNERS routes protected review to `@amballin`; the active GitHub ruleset is what prevents another account from updating `main` without that owner.
...
**Profile Packs & Sharing** keeps pack creation, extension, Git, and computer guidance out of the daily path. Step 4C allows the ordinary editor launcher to use a private machine-local saved pack, Steps 5A–5B pass that exact pack to Camera Lab for comparison and guarded camera operation, Step 5C permits reviewed evidence promotion, Step 6A provides guarded New Profile Pack creation from embedded sources, Step 7A applies the selective validated starter policy, Step 7B adds missing official profiles later through a guarded add-only transaction, Step 7C presents and verifies that action as **Add Profiles from Catalog**, and Step 6B provides separately reviewed pack Git and read-only combined handoff status. The embedded source remains authoritative. Direct Camera Lab pack-source writes, source-migration activation, and external-pack publication remain future work.
