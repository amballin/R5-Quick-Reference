...
Camera Lab never updates the Setup & Verification Tracker automatically. In **Review & Build**, open **Camera Lab evidence** to list exact unmigrated setting evidence from completed physical-camera sessions. Nothing is checked automatically. The first importer can mark only the matching current C1–C3 setting as configured; it does not mark read-back, registration, operational testing, backups, Canon capability, or owner-confirmed project evidence. It excludes simulator and incomplete sessions, records the journal provenance, blocks when the workbook has unimported edits, and requires exact YAML review plus a separate confirmation.
...
Open **Review & Build** to validate the editing session before generation. Save each intended draft through its guarded review or choose **Discard** and confirm. The local build and Camera Lab evidence review remain locked while the pending list is nonempty. For Camera Lab evidence, refresh the list, check only the exact items to retain, review the canonical status diff, acknowledge the narrow evidence boundary, and import; the safely synchronized working tracker is rebuilt automatically. When a manually edited verification workbook contains results to retain, save and close Numbers or Excel, choose **Import verification tracker**, and confirm. Neither path imports automatically. Then run a fresh readiness check before the local build.
