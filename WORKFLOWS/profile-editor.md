...
**Setup & Sharing** keeps fork and computer guidance out of the daily path. Step 4C allows the ordinary editor launcher to use a private machine-local saved pack, Steps 5A–5B pass that exact pack to Camera Lab for comparison and guarded camera operation, and Step 5C permits reviewed evidence promotion. Source migration, direct Camera Lab pack-source writes, two-repository Git/handoff, and publication activation remain future work. A guided **New Profile Pack** wizard is recorded for a later separately approved step and is not available yet.
