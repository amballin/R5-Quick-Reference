...
Profile Editor 1.0 is the project's main local interface for routine work: a Start → Work → Finish daily path, shooting profiles, persisted My Menu organization and card colors, baseline setup, session review, local builds, guarded Finish Day handoff, optional guarded branch integration, and camera reference. It provides a Canon-sourced EOS R5 still-photo settings dictionary, runs the existing preflight as a quiet opening check, loads and saves the reviewed My Menu layout, renders isolated shooting and reference-card previews, can create, duplicate, or update shooting-profile YAML, can apply a complete reviewed baseline migration, can run the guarded normal local build after every browser draft is resolved, and can separately confirm reviewed Git stages without publishing.
...
The in-app workspace and `finish-day.sh` share `80 Build/finish_day.py`; neither is a second implementation of the workflow. **Check** verifies zero browser drafts, current verification state, the current branch, its exact same-named `origin` upstream, and refreshed ahead/behind state. **Prepare** requires confirmation before running source validation, the normal build, full validation, and backed-up removal of generated `docs/` changes from the handoff. **Commit** requires the exact source list, a content-sensitive one-use review, a message, and separate confirmation. **Push** requires another confirmation, refuses outgoing `docs/` changes, pushes only the reviewed current branch to its matching upstream, and verifies the final clean synchronized state.

### Integrate a finished branch

Use **Integrate Branch** only after Finish Day reports that the current non-main branch is clean and synchronized. **Check** refreshes `origin`, requires the branch's exact same-named upstream, and targets only `origin/main`. **Review** creates the proposed merge in a disposable worktree, stops cleanly on conflicts, rejects `docs/` changes, runs source validation, the normal build, and full validation, restores generated website files, and shows the exact commits and files.
...
**Release & Publish** is separate because publication is not an everyday action. Approved prototype work must be integrated into `main` before publication, and the publisher stops on any other branch. The workspace presents the established release gates and links to the authoritative guides without running Git or publishing from the browser.
...
The editor intentionally provides no released-profile removal, permanent deletion, existing-profile rename, direct baseline edit, reference-card edit, publish, or website-version action. Unreleased editable profiles may be moved only through the UUID dependency-checked, exact-diff, backed-up Deleted Cards transaction and restored through its matching guarded review. Baseline writes are available only through a complete reviewed migration. My Menu layout and colors are available only through their exact-diff guarded transaction. Review & Build offers the existing guarded tracker importer plus the guarded local build sequence. Commit and branch push exist only in Finish Day. Merge to main, main push, and branch resynchronization exist only in Integrate Branch. Each authority requires its own confirmation, generated `docs/` changes are rejected, and publication remains a separate established workflow.
