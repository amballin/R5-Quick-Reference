...
- Use validation proportional to the affected surface. A targeted check or source-only validation is sufficient when a narrow source change cannot affect generated behavior. Run the normal source-validation, development-build, and full-validation sequence when generated content or build behavior can change and at integration, computer-handoff, and Finish Day checkpoints. Publication uses the supported publish-mode build and verification and does not require a separate unchanged development build immediately beforehand. Do not repeat an unchanged full build unless inputs changed, a prior check failed, or deterministic generation is itself under test.
- Isolated Camera Lab iteration may use the streamlined validation loop defined by the USB Camera Configuration Specification. This exception never replaces the normal source-validation, development-build, and full-validation sequence at the integration, Git handoff, or Finish Day checkpoints defined there, or the supported publish-mode build and verification at publication.
...
- [Application Owner Authorization Specification](00%20Master/specifications/Application%20Owner%20Authorization%20Specification.md)
