...
2. **Review** shows the same reconnectable live command progress while it creates the proposed merge in a disposable worktree based on current `origin/main`. It stops without changing either real branch on conflicts and rejects proposed `docs/` changes. If that exact merged candidate needs safe spreadsheet regeneration, use the stopped panel's permission checkbox and **Refresh spreadsheets and retry**; the refresh runs only inside a fresh disposable candidate and Apple Numbers runs temporarily in the background. If Numbers was already open, save and close it and use the same retry control. It then runs source validation, the normal development build, and full validation, restores generated website files, and displays the exact commits and files. If protected application-catalog files change, also review the exact protected YAML diff and complete the separate application-owner approval; general merge approval cannot bypass it.
3. **Merge Main** requires confirmation and applies the exact reviewed tree to a clean local `main`. For a protected catalog candidate, this remains disabled until the separate owner approval is recorded against that exact candidate and both reviewed heads. Nothing is pushed.
...
After the Step 7D ownership contract first reaches `main`, complete the one-time [Application Owner Protection](WORKFLOWS/application-owner-protection.html) setup. CODEOWNERS supplies owner review routing; the active GitHub ruleset supplies remote enforcement.
...
This diagnoses the local verification working copy plus both release families, safely refreshes only stale artifacts in dependency order, and verifies the Subject Settings Matrix and EOS R5 Setup & Verification Tracker in Excel and Apple Numbers. Numbers runs temporarily in the background, closes generated workbooks, and quits automatically. If Numbers is already open, save and close it and rerun the same command. The workbook files are machine-local and are not committed to Git.
