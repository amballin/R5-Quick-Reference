...
- Profile choices and comparison headings must identify the intended registered-mode foundation by C1–C3 and its base card title. In the selector, list the three registered bases first in slot order as `C1 (Wildlife)`, `C2 (Birds in Flight)`, and `C3 (Landscape)`. List every remaining profile alphabetically as `Profile ← Cx (Base)`, for example `Sports ← C2 (Birds in Flight)`; use `Profile ← No Cx` when no foundation is assigned. Comparison headings retain the base-first form `C1 – Wildlife` for a registered base and `C1 – Wildlife → People` for a derived card so the camera starting position and selected comparison target remain distinct.
