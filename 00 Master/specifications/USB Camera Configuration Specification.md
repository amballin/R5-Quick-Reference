...
- Steps 5A and 5B may launch Camera Lab from Profile Editor with one explicit compatible external profile-pack context. The Lab must use the centralized resolved sources, display the live path-free pack identity, and place default journals, confirmations, qualifications, write evidence, and browser checklist state under that pack ID's machine-local namespace. It must not scan for packs or accept a pack path through the browser. Step 5C may expose those completed physical journals to the same active pack's Profile Editor evidence review.
- External-pack Camera Lab may use the normal guarded simulator and the explicitly enabled physical-write mode, including reversible qualification, under every existing preflight, review, confirmation, identity, readback, and stop-on-failure safeguard. Every API operation revalidates the external pack identity and fingerprint. New guarded and qualification journals record path-free pack identity; a legacy journal without that identity cannot resume or become promotion evidence for an external pack. Camera Lab itself must not write profile-pack source or promote local evidence into canonical verification state, while authenticated shutdown remains available for recovery.
- Camera Lab's version summary is the visible Main/Prototype application-context signal. Its expandable detail retains the full branch and source hash; no separate project-context badge is shown. The friendly path-free `Profile Pack:` badge remains visible independently.
...
Promotion must be blocked while the active pack's machine-local verification workbook has unimported edits. With embedded sources, the reviewed canonical status change and safely rebuilt working tracker use the guarded transaction requirements in the Build and Validation Specification. Under Step 5C, external inventory reads only the active pack ID's machine-local guarded-run namespace, requires matching recorded pack identity, and exposes no journal path. The confirmed transaction writes only the active pack's manifest-owned verification-status YAML and deliberately does not generate or refresh spreadsheet files. Camera Lab itself continues to write no repository source or tracker status.
...
Reopening a responsive Profile Editor or Camera Lab application must recover its Chrome window instead of attempting a duplicate server. Camera Lab reuse requires a successful loopback status response identifying a valid Camera Lab backend, application version/context, and the exact requested pack ID; it may be read-only or explicitly write-enabled. A listener that does not pass that check remains an unrecognized port conflict and must not be stopped or reused automatically.
...
- Steps 5A–5B external-pack Camera Lab: implemented as an exact selected-pack handoff with scan, comparison, guarded simulation, and explicitly enabled physical-camera operation. The Lab displays the manifest name, loads inputs through `ProjectPaths`, isolates local state and journals by pack ID, records pack identity, refuses reuse across different pack IDs, and revalidates independent changes. On September 4, 2026, a physical EOS R5 regression with firmware 2.2.1 and EDSDK 13.20.20.0 passed the complete reversible Picture Style Standard-to-Neutral write, target readback, restoration, and restoration-readback sequence in both embedded and external-pack contexts; the external journal recorded the active pack identity and no application or pack source changed. Direct Camera Lab pack-source writes remain unavailable.
- Step 5C reviewed tracker promotion: implemented in Profile Editor for embedded and external completed physical EDSDK guarded-run journals. It selects nothing automatically and promotes only exact mapped setting evidence into the currently assigned C1–C3 slot's configured result, with journal provenance and deduplication. External inventory is pack-ID-matched and path-free, and its reviewed transaction changes only pack-owned verification status without generating a spreadsheet. Simulator results, incomplete or foreign-pack journals, and broader verification claims are excluded; read-back, registration completion, operational tests, backups, Canon capability claims, and owner-confirmed project evidence remain separate.
