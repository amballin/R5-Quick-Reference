...
- The owner application fork treats every embedded `10 Profiles/*.yaml` declaration as its protected application profile catalog. Matching entries in the embedded `00 Master/profile_lens_guidance.yaml`, keyed by those cards' immutable IDs, are protected catalog accompaniment. A shared upstream may propose changes to either source family, but ordinary inbound integration must not accept them silently or mix them into an otherwise routine application update; the application owner must receive and explicitly approve a separate exact catalog review.
- A textual owner claim is never identity proof. Before Profile Editor is distributed to other profile-pack owners, owner-only embedded writes, all application `main` integration and push operations, application publication mutations, owner-policy changes, and protected shared-upstream acceptance must require a short-lived cryptographic authorization signed by the application owner's dedicated external key and bound to the exact reviewed operation. GitHub repository permissions and the active ruleset remain an independent remote enforcement layer.
- Until source migration activation and broader external-pack workflows are separately approved, the current Git root remains the application authority and embedded compatibility sources remain available. The development build and general validator may compose one explicitly selected external pack for isolated parity review. Profile Editor Step 4C may use either an explicit or machine-local saved pack, switch among remembered packs and embedded sources after draft/confirmation checks, and perform guarded reviewed transactions only against the active pack's manifest-owned sources. Steps 5A–5B pass that exact context into Camera Lab for scan, comparison, guarded simulation, and explicitly enabled physical-camera operation. Step 5C lets Profile Editor deliberately promote exact completed physical-camera evidence into the pack-owned verification status. Step 6A creates, validates, registers, and selects a new external pack; Step 7A constrains it to the required editable C1–C3/reference seed plus explicitly selected optional catalog profiles and resets inherited camera evidence. Step 7B may add absent official application-catalog subject profiles and matching lens guidance to an existing external pack through an add-only, reviewed, backed-up, validated transaction that cannot overwrite or remove pack source. Step 6B permits only separately reviewed Git actions in that pack and read-only combined handoff checks across the application and pack. Camera Lab direct pack-source writes, editor-initiated build, cleanup, application Git mutation from the external session, spreadsheet-generation, main-editor launch, and publication workflows remain unavailable.
...
Step 7E authenticated owner-authorization flow:

```text
exact reviewed application operation + current trusted origin/main owner policy
                              -> canonical short-lived authorization statement
                              -> signature by external application-owner private key
                              -> server-side verification against current trusted public key
                              -> one-use operation-bound in-memory authorization receipt
                              -> fresh candidate/ref/diff verification at mutation boundary
                              -> local owner-only mutation
                              -> independent GitHub authentication and main-ruleset enforcement
```
...
- `00 Master/specifications/Application Owner Authorization Specification.md` defines the accepted but not yet implemented Step 7E authentication mechanism, protected operation classes, key policy, failure/recovery rules, and release gate for distribution to other profile-pack owners.
