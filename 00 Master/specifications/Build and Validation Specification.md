...
- `./80 Build/scripts/build-settings-downloads.sh` is the dedicated on-demand spreadsheet workflow. It generates only the Excel summary, converts it to Apple Numbers, finalizes both native frozen headers and the Numbers layout, and records the readiness manifest without running the website build.
- `./80 Build/scripts/prepare-settings-downloads.sh` remains the manual conversion fallback. It generates only the Excel summary without running the website build, opens it in Numbers, and prints the exact `.numbers` save destination. After the imported workbook is saved there, `./80 Build/scripts/prepare-settings-downloads.sh --verify` removes the four import-only rows from the Numbers table, positions the single banner image above the table, assigns and freezes one native Numbers header row, assigns and freezes columns A:C as native header columns, and records a machine-local content-hash manifest for the exact Excel and Numbers files.
- `./80 Build/scripts/publish.sh --settings-downloads` is the opt-in publication path for both verified workbook formats. It refuses missing, stale, or changed files. The option may be combined with `--png`; normal publishing omits workbook downloads.
...
- A verified `Build Output/reports/Subject Settings Matrix.numbers` and its machine-local readiness manifest may accompany the Excel report. When opt-in publication is authorized, the generated site contains `downloads/Subject Settings Matrix.xlsx` and `downloads/Subject Settings Matrix.numbers` plus corresponding links under the **Downloads** section in a **Subject Settings Matrix** row. The readiness manifest is never published.
...
- consistency of a prepared workbook-download manifest and any generated site downloads (`settings_download_validator.py`);
