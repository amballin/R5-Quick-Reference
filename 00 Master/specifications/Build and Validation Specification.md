...
- `./80 Build/scripts/publish.sh --matrix-downloads`, `--setup-downloads`, or `--spreadsheet-downloads` includes only the requested, verified workbook families. `--settings-downloads` remains a Matrix compatibility alias. Each option refuses missing, stale, or changed files and may be combined with `--png`; normal publishing omits workbook downloads.
...
- A verified Numbers companion and a separate content-hash manifest may accompany each Excel report. When opt-in publication is authorized, the generated site contains the requested Excel/Numbers pairs under **Downloads**. Readiness manifests and migrated Setup working copies are never published.
...
- consistency of prepared workbook-download manifests and any generated site downloads (`spreadsheet_download_validator.py`);
