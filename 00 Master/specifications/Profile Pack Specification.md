...
- Step 6B exposes private-pack Git only in Setup & Sharing with an external pack selected. Every mutating action requires zero pending browser drafts, exact current pack identity, and its own explicit review or confirmation.
...
- Setup & Sharing presents this work as four state-driven steps: create or select, save locally, connect private GitHub, and push and verify. Only the current action is expanded; completed steps remain visibly complete, and ownership diagnostics remain available as optional detail.
...
- A status refresh or reopened Setup & Sharing view must reconcile the saved receipt with the current live-head check. When the pack is already synchronized, it displays the current verified commit and must not retain stale instructions to create a repository, configure an origin, or push again.
...
Step 6A adds a guided **New Profile Pack** capability to Profile Editor's Setup & Sharing workspace. It collects a user-friendly `pack_name`; the authenticated backend then opens the native macOS Save panel so the owner can use normal Finder navigation and New Folder support to choose an exact absolute destination. The destination field is not a free-text path control. Creation requires embedded sources and zero pending browser drafts, and creates a one-use review containing the new immutable UUID, exact manifest, migrated source inventory, destination, and local-Git boundary without writing anything. The destination's parent must exist; the destination itself must not exist, must be outside both repositories, and must pass the prohibited-name policy. The editor never scans for a destination or pack.

After separate confirmation, creation rechecks the embedded fingerprint, stages the complete version-1 source structure and application-owned pack `AGENTS.md`, initializes a local Git repository, runs combined source validation, and atomically installs the new destination. It then constructs the complete editor model and only afterward registers and selects the pack. A failed source recheck, copy, Git initialization, validation, editor load, or registration must leave embedded source unchanged and remove only the new staged or installed destination. Git initialization creates no commit, configures no remote, pushes nothing, and does not activate a general pack Git workflow. Creation controls remain unavailable while an external pack is active, but Setup & Sharing remains accessible and provides the normal guarded switch to embedded sources.
...
- general validation may use the explicit Step 3B pack argument, and Profile Editor may use explicit or machine-local Step 4C selection for guarded manifest-owned editing and pack-namespaced preview, backup, removal, and restore state; Steps 5A–5B Camera Lab may use that selected external pack for comparison and guarded camera operation, Step 5C Profile Editor may promote exact eligible evidence into its verification status, Step 6A may create and select a validated migrated-copy pack from embedded sources, and Step 6B may operate the separately reviewed private-pack Git and combined-handoff workflow; equipment editing, independent verification editing, Camera Lab direct pack-source writes, spreadsheet generation, editor-initiated build, application Finish Day, Integrate Branch, publication, and cleanup remain embedded-only;
- Setup & Sharing must describe the active Step 4C editor selection, Steps 5A–5C Camera Lab/evidence boundary, Step 6A guarded pack creation, and Step 6B independent pack Git/handoff, while clearly marking source-migration activation, direct Camera Lab pack-source writes, and publication activation as future work.
