...
This specification defines the accepted target contract for composing an independently owned private profile pack with the Canon EOS R5 application repository. It governs pack identity, source ownership, compatibility, selection, privacy, publication, Git independence, migration, and activation. General system boundaries remain governed by [`Architecture.md`](Architecture.md); authenticated owner-only application mutations and the distribution release gate are governed by [`Application Owner Authorization Specification.md`](Application%20Owner%20Authorization%20Specification.md); profile YAML semantics remain governed by [`Profile Specification.md`](Profile%20Specification.md).
...
- Before distribution to other profile-pack owners, the application-owner role must be authenticated under the Application Owner Authorization Specification for every owner-only application mutation. Pack ownership, application file possession, editor access, and general confirmation never confer that authority.
