...
Profile cards may define `card.field_setup` to identify an intended C1–C3 starting configuration and the My Menu tabs that provide fast access to listed settings. `start` is `C1`, `C2`, or `C3`; `source_profile` is the exact profile title represented by that registered mode; optional `my_menus` contains at most five named tabs, each with a non-empty list of unique card-setting paths. A baseline-driven profile displayed in the reference category may instead set `access_only: true`, omit `start` and `source_profile`, and use the route solely to identify My Menu access without implying a registered C-mode. Access-only routes require at least one authored My Menu tab. A setting may belong to only one displayed My Menu tab on a card.
...
- For every profile card with a starting Cx, compare each visible merged row with the effective values of the exact profile named by `card.field_setup.source_profile`. Reserve a fixed rightmost change-indicator column. Leave its cell blank when all underlying settings represented by that row match the foundation; otherwise show `Δ` in the same color as that row's displayed setting value. For combined Track / Accel, IBIS/Lens IS, and ISO rows, show `Δ` when any represented source setting differs. Keep My Menu coloring independent from the change calculation. Include a visible legend and an accessible label identifying the selected Cx foundation. Access-only and permanent reference cards do not render the Cx-change column. Change markers are derived rendering state and must not be authored in profile YAML.
...
- Prepend a generated Notes item explaining the full starting profile, the need to verify its Cx registration when applicable, that My Menu colors identify access regardless of whether a change is required, and that a same-colored `Δ` identifies a value that differs from the starting foundation. Access-only routes explain the color distinction without claiming a C-mode starting profile. Support multiple My Menu tabs without requiring them on current cards.
