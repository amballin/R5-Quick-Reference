...
Cards use `card_type: profile` by default. Profile cards inherit the baseline and render merged camera settings. Permanent reference cards use `card_type: reference`, do not inherit the baseline or define overrides, and normally render their `reference_settings` entries as a two-column **Settings** table. A derived reference card declares `reference_source` and must not author `reference_settings`: `my_menu` derives tab sections and ordered items from the persisted My Menu configuration, while `controls` derives Camera Buttons assignments and details from `controls.yaml`. The same canonical control source generates the complete controls and dials tables wherever their markers appear in the Custom Controls appendix and Deep Dive; authored prose remains outside those generated table markers.
...
Each authored `reference_settings` entry contains a non-empty `control` and `assignment`. Reference-card checklists contain concise recommendations or verification steps rather than repeating the Settings table.
...
- On each subject card, render **Lens Choices** immediately after Settings. Resolve one primary and up to two alternative or specialist choices from `00 Master/profile_lens_guidance.yaml` and `data/stabilization_reference.yaml`. Each choice shows the owned lens/accessory name, role, when to use it, and the immediate field check. Do not show this section on Camera Defaults, setup/reference cards, or profiles without governed lens guidance, and never imply that a listed lens is currently attached.
- Render **Compatibility** immediately after Lens Choices and before Checklist. Include de-duplicated profile-setting rules plus rules evaluated separately for every listed lens/accessory context. Clearly identify conditional lens behavior in the rule text, including the available Control Ring Mount Adapter ring, extender tradeoffs, optical/body IS behavior, MP-E manual-focus limits, and EF-S forced crop. Omit the entire section when no rule matches. Keep authored advice and links under Notes.
- Normal profile-card rows follow `card_layout.display_order`, which mirrors the conceptual sequence of the R5 Quick Reference. Authored reference-card rows retain the explicit order of their `reference_settings` list; derived Camera Buttons rows retain canonical `controls.yaml` control order followed by dial order.
- Subject profile cards with a C1–C3 foundation begin with a compact **Change from Cx [source card]** section containing only rows that differ from that foundation. Group those rows for a single efficient pass through the camera: buttons, dials, and physical switches; Q screen; each saved My Menu tab in persisted tab and item order; then standard Canon menu pages in menu-family and page order. When no row differs, state that no camera changes are needed from the selected foundation. Preserve the complete shooting-oriented **Settings** table beneath this route.
- Camera Defaults and Camera Setup Essentials render their complete visible settings as **Rapid Camera Setup** in the same physical-control, Q-screen, saved-My-Menu, and standard-menu-page sequence. This ordering is derived rendering state from `setting_access.yaml`, the persisted My Menu configuration, and the Canon settings catalog; do not duplicate it in profile YAML. Preserve the change-indicator behavior and legend on these setup cards.
...
- In Profile Editor's desktop two-pane layout, fit the preview iframe within the available sticky panel height after its heading and status content so the complete card remains independently scrollable. On narrow layouts, retain the dedicated Preview pane with an ordinary viewport-height iframe.
...
- `00 Master/feature_interactions.yaml` and `80 Build/feature_interactions.py` supply validated conditional Compatibility notes without adding interaction prose to profile YAML.
- `00 Master/profile_lens_guidance.yaml`, `data/stabilization_reference.yaml`, and `80 Build/lens_guidance.py` supply ordered Lens Choices and candidate-lens contexts without adding equipment recommendations to profile YAML.
