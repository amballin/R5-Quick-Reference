...
The operational baseline and Camera Defaults reset state intentionally match the approved C1 Wildlife target, including explicit Auto shutter and aperture values, Servo AF Case A (Auto), Face + Tracking, Animals, Eye Detection enabled, and High Speed Continuous. Birds in Flight and Sports override the Case with Case 4; One-Shot and Manual Focus profiles retain the stored Case but do not actively use or display it. Profiles for deliberate-point, static, manual-focus, or specialized Servo work override only the inherited choices they change. This alignment is an approved target pending physical verification and does not establish that the camera currently matches it.
...
Servo AF cards show the effective Servo AF Case directly after AF Operation. Registered C1 Wildlife supplies Case A (Auto), registered C2 Birds in Flight supplies Case 4, and C3 stores Case A without using it in its One-Shot starting state. My Menu: AF Case contains Servo AF, Tracking Sensitivity, and Accel./Decel. tracking; Servo AF opens the Case selector. Wildlife, Birds in Flight, Birds Perched, People, and Sports associate their displayed Servo AF Case with that tab. AF3 > Servo AF characteristics remains the direct menu route.
