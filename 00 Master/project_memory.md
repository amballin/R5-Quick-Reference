...
Applicable subject cards carry compact field-access metadata for the intended starting C1–C3 mode and any My Menu tabs used for remaining changes. A shared curated palette assigns colors by named My Menu tab, colors matching setting values and their change indicators, and leaves Quick Control, dial, and button values white. SWITCH starts green and AF Case starts gold but both may be deliberately reassigned through the guarded My Menu review. The persisted tab names and ordered shortcuts also generate a read-only My Menu reference card for field use; the card follows every saved used tab without duplicating its contents in profile YAML. Light Red is intentionally redder than the orange-red Coral choice. The full explanation remains in generated Notes so the top-of-card route stays field-ready.
