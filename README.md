...
The apps are written to the sibling `<repository folder name> Local/Applications/` folder. Both apps run without opening Terminal and record diagnostic output under the machine-local `Logs/` folder. The main Profile Editor uses port 8765; a development-worktree editor uses port 8766 and a distinct macOS application identity so both can run together. Reopening either already running app recovers its Chrome window. The repository's **Stop Profile Editor.command** is the ownership-checked fallback for a hidden or unresponsive editor. Rebuild the apps after moving or renaming the repository, or when setting up another Mac.

Profile Editor and Camera Lab share a local-application version displayed as `Major.Minor.Incremental · Main/Prototype`. Major is manually controlled, Minor advances with Git commits, and Incremental advances once per completed development update between commits. Complete an approved update with `./80\ Build/scripts/complete-development-update.sh`; do not run that command repeatedly for the same update. Expand the version badge in either app to see its diagnostic source hash.
...
`preflight-git.sh` refreshes `origin`, accepts the checked-out `main` or prototype branch only when it tracks its exact same-named upstream, and blocks work when the clone is behind or diverged. Profile Editor's **Finish Day** workspace and `finish-day.sh` use one shared guarded engine to validate, build, back up and separate generated `docs/`, review source changes, commit, and push the current branch. Browser preparation shows reconnectable command progress, elapsed time, completed steps, and bounded output. Commit and push require separate confirmations. The workflow refuses outgoing `docs/` changes, so a prototype handoff pushes only its matching prototype branch and cannot publish Pages. The separate **Integrate Branch** workspace validates a finished branch in a disposable worktree with the same visible progress, reviews and merges its exact tree into local `main`, pushes `main`, and fast-forwards the working branch; every mutation has its own confirmation and none invokes publication. Optional **Cleanup Review** protects the newest successful recognized backups, selects nothing automatically, and permanently deletes only exact separately confirmed candidates. The live publisher runs only from `main`. Do not switch Macs until the final report says the working tree is clean and synchronized. See [`HOW_TO.md`](HOW_TO.md) for the full workflow.

Start with the local [`Workflow Index`](WORKFLOWS/index.html). The [`Profile Editor User Guide`](WORKFLOWS/editor-user-guide.html) covers routine profile, lens-choice, My Menu, baseline, camera-reference, validation, local-build, visible guarded Finish Day and branch-integration progress, and optional exact cleanup review. The [`Camera Lab User Guide`](WORKFLOWS/camera-lab-user-guide.html) covers physical-camera comparison, simulator rehearsal, C1–C3 maintenance, and guarded profile application; the same guide opens from the Lab header. The index retains specialized workflows for spreadsheets, physical-camera testing, publishing, computer handoff, and recovery. For the short end-of-day recipe—including source commit/push, optional branch integration and cleanup, both spreadsheet families, publication, and the final synchronization check—open [`FINISH_DAY.html`](FINISH_DAY.html). Normal builds and Finish Day automatically refresh all tracked HTML workflow pages from their Markdown sources. These project-help pages are synchronized through Git but are never copied into the published website.

The machine-local USB camera configurator begins with a read-only EOS R5 connection probe. Canon EDSDK remains an external machine dependency and is never stored in this repository. See [`USB Camera Configuration`](WORKFLOWS/usb-camera-configuration.md) for installation-path configuration, development, and the smoke-test command.
