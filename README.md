...
`preflight-git.sh` refreshes `origin`, accepts the checked-out `main` or prototype branch only when it tracks its exact same-named upstream, and blocks work when the clone is behind or diverged. Profile Editor's **Finish Day** workspace and `finish-day.sh` use one shared guarded engine to validate, build, back up and separate generated `docs/`, review source changes, commit, and push the current branch. Commit and push require separate confirmations. The workflow refuses outgoing `docs/` changes, so a prototype handoff pushes only its matching prototype branch and cannot publish Pages; the live publisher runs only from `main`. Do not switch Macs until the final report says the working tree is clean and synchronized. See [`HOW_TO.md`](HOW_TO.md) for the full workflow.

Start with the local [`Workflow Index`](WORKFLOWS/index.html). The [`Profile Editor User Guide`](WORKFLOWS/editor-user-guide.html) covers routine profile, lens-choice, My Menu, baseline, camera-reference, validation, local-build, and guarded Finish Day work. The [`Camera Lab User Guide`](WORKFLOWS/camera-lab-user-guide.html) covers physical-camera comparison, simulator rehearsal, C1–C3 maintenance, and guarded profile application; the same guide opens from the Lab header. The index retains specialized workflows for spreadsheets, physical-camera testing, publishing, computer handoff, and recovery. For the short end-of-day recipe—including source commit/push, both spreadsheet families, publication, and the final synchronization check—open [`FINISH_DAY.html`](FINISH_DAY.html). Normal builds and Finish Day automatically refresh all tracked HTML workflow pages from their Markdown sources. These project-help pages are synchronized through Git but are never copied into the published website.
